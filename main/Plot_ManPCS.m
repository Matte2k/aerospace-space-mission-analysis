...
run('ManovraPCS.m')                         % Calcolo dati manovra
...
if (theta_i < theta_2)
    theta_vect_i = (theta_i:offset_Plot:theta_2)';
elseif (theta_i > theta_2)
    theta_vect_i = (theta_i:offset_Plot:(2*pi + theta_2))';
elseif (theta_i == theta_2)
    theta_vect_i = 0;
end
...
if (theta_2 < theta_3)
    theta_vect_man1 = (theta_2:offset_Plot:theta_3)';
elseif (theta_2 > theta_3)
    theta_vect_man1 = (theta_2:offset_Plot:(2*pi + theta_3))';
elseif (theta_2 == theta_3)
    theta_vect_man1 = 0;
end
...
for k_1 = 1:length(theta_vect_man1)
    [rr,vv] = ParOrb2RV(a_i,e_i,i_f,OM_f,om_2,theta_vect_man1(k_1), mu_Earth);
    R_1(:,k_1)=rr;
    V_1(:,k_1)=vv;
end
...
for k_1 = 1:length(theta_vect_cmpl)
    [rr,~] = ParOrb2RV(a_i,e_i,i_f,OM_f,om_2,theta_vect_cmpl(k_1), mu_Earth);
    R_1_cmpl(:,k_1)=rr;
end
...
%% CALCOLI on ORBITA CIRCOLARE

delta_theta = om_3 - om_2 ;
theta_end_circ = theta_3 + delta_theta;
[theta_end_circ] = RifasAngle(theta_end_circ);


% Vettore Theta Manovra_circ
if theta_3 < theta_end_circ
    theta_vect_mancirc = (theta_3:offset_Plot:theta_end_circ)';
elseif theta_3 > theta_end_circ
    theta_vect_mancirc = (theta_3:offset_Plot:(2*pi+theta_end_circ))';
elseif (theta_3 == theta_end_circ)
...
for k_circ = 1:length(theta_vect_mancirc)
    [rr,vv] = ParOrb2RV(r_anChange,0,i_f,OM_f,om_2,theta_vect_mancirc(k_circ), mu_Earth);
    R_circ(:,k_circ)=rr;
    V_circ(:,k_circ)=vv;
end
...
for k_circ = 1:length(theta_vect_cmpl)
    [rr,~] = ParOrb2RV(r_anChange,0,i_f,OM_f,om_2,theta_vect_cmpl(k_circ), mu_Earth);
...
if (Start_Bitan == 1)
    if theta_3 < pi
        theta_vect_man2 = (theta_3:offset_Plot:pi)';
    elseif theta_3 > pi
        theta_vect_man2 = (theta_3:offset_Plot:(2*pi+pi))';
    elseif (theta_3 == pi)
        theta_vect_man2 = pi;
    end
    
elseif (Start_Bitan == 0)
    if (theta_3 == 0) || (theta_3 == 2*pi)
        theta_vect_man2 = 0;
    else
...
for k_2 = 1:length(theta_vect_man2)
    [rr,vv] = ParOrb2RV(a_i,e_i,i_f,OM_f,om_3,theta_vect_man2(k_2), mu_Earth);
    R_2(:,k_2)=rr;
    V_2(:,k_2)=vv;
end
...
for k_2 = 1:length(theta_vect_cmpl)
    [rr,~] = ParOrb2RV(a_i,e_i,i_f,OM_f,om_3,theta_vect_cmpl(k_2), mu_Earth);
    R_2_cmpl(:,k_2)=rr;
end
...
if (Start_Bitan == 0) && (Start_AnPeri == 0)            % peri-apo
    theta_vect_man3 = (0:offset_Plot:pi)';
    theta_start_f = pi;
    om_t = om_3;
    
elseif (Start_Bitan == 1) && (Start_AnPeri == 0)        % apo-peri
    theta_vect_man3 = (pi:offset_Plot:2*pi)';
    theta_start_f = 0;
    om_t = om_3;
...
for k_3 = 1:length(theta_vect_man3)
    [rr,vv] = ParOrb2RV(a_t,e_t,i_f,OM_f,om_t,theta_vect_man3(k_3), mu_Earth);
    R_3(:,k_3)=rr;
    V_3(:,k_3)=vv;
end
...
for k_3 = 1:length(theta_vect_cmpl)
    [rr,~] = ParOrb2RV(a_t,e_t,i_f,OM_f,om_t,theta_vect_cmpl(k_3), mu_Earth);
    R_3_cmpl(:,k_3)=rr;
end
...
%% CALCOLI from ORIBITA 2 to Pt.FINALE
% Vettore Theta Orbita_f
if (theta_start_f < theta_f)
    theta_vect_f = (theta_start_f:offset_Plot:theta_f)';
elseif (theta_start_f > theta_f)
    theta_vect_f = (theta_start_f:offset_Plot:(2*pi + theta_f))';
elseif (theta_start_f == theta_f)
    theta_vect_f = 0;
end
...
plot3(X_Orbita_i_cmpl,Y_Orbita_i_cmpl,Z_Orbita_i_cmpl,'LineWidth',2.0,'LineStyle',':','Color','r')      % Orbita completa
...
% Plot Orbita_cric 3D
plot3(X_Orbita_circ,Y_Orbita_circ,Z_Orbita_circ,'LineWidth',2.0,'Color','y')                                     % Orbita percorsa
plot3(X_Orbita_circ_cmpl,Y_Orbita_circ_cmpl,Z_Orbita_circ_cmpl,'LineWidth',1.0,'LineStyle',':','Color','y')      % Orbita completa
plot3(X_Orbita_circ(1),Y_Orbita_circ(1),Z_Orbita_circ(1),'o','MarkerEdgeColor','y')                              % Punto inizio percorso
