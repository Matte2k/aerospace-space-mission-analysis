...
run('Plot_ManCB.m')                        % Plot statico manovra

% Vettore Theta Orbita_tot percorsa
theta_vect_tot = [theta_vect_mancirc;theta_vect_man1;theta_vect_man2;theta_vect_man3;theta_vect_f];

% Vettore numero di frame
theta_vect_frame = zeros(length(theta_vect_tot),1);

% Condizioni di cambio orbita
change_plotmancirc = length(theta_vect_mancirc);
change_plot1 = change_plotmancirc + length(theta_vect_man1);
...
X_Orbita_tot = [X_Orbita_circ,X_Orbita_1,X_Orbita_2,X_Orbita_3,X_Orbita_f];
Y_Orbita_tot = [Y_Orbita_circ,Y_Orbita_1,Y_Orbita_2,Y_Orbita_3,Y_Orbita_f];
Z_Orbita_tot = [Z_Orbita_circ,Z_Orbita_1,Z_Orbita_2,Z_Orbita_3,Z_Orbita_f];
...
plot3(X_Orbita_i_cmpl,Y_Orbita_i_cmpl,Z_Orbita_i_cmpl,'LineWidth',1.0,'LineStyle',':','Color','k')      % Orbita_i completa
%plot3(X_Orbita_i(1),Y_Orbita_i(1),Z_Orbita_i(1),'o','MarkerEdgeColor','r','MarkerFaceColor','#ffa500')          % Punto iniziale
plot3(X_Orbita_circ_cmpl,Y_Orbita_circ_cmpl,Z_Orbita_circ_cmpl,'LineWidth',1.0,'LineStyle',':','Color','r')      % Orbita_circ completa
plot3(X_Orbita_circ(1),Y_Orbita_circ(1),Z_Orbita_circ(1),'o','MarkerEdgeColor','r')                              % Punto inizio percorso Orbita_circ
...
for k_frame = 1:length(theta_vect_frame)    
    ...
    if (k_frame < change_plotmancirc)    
        ...
        [~,V_orb_i] = ParOrb2RV(r_circ,0,i_circ,OM_circ,0, theta_vect_tot(k_frame), mu_Earth);
    ...
    elseif (k_frame > change_plotmancirc) && (k_frame < change_plot1)
        % Plot orbita_1
        plot3(X_Orbita_tot(1,change_plotmancirc:k_frame),Y_Orbita_tot(1,change_plotmancirc:k_frame),Z_Orbita_tot(1,change_plotmancirc:k_frame),'Color','g','LineWidth',2.0)
        
        % Plot vettore velocità satellite su orbita_1
        [~,V_orb_i] = ParOrb2RV(a_b,e_b,i_circ,OM_circ,om_b, theta_vect_tot(k_frame), mu_Earth);
        V_vector = quiver3(X_Orbita_tot(k_frame),Y_Orbita_tot(k_frame),Z_Orbita_tot(k_frame),V_orb_i(1),V_orb_i(2),V_orb_i(3),1000,'Color','k','LineWidth',2.0);
        
    elseif (k_frame > change_plot1) && (k_frame < change_plot2)
        % Plot orbita_2
        plot3(X_Orbita_tot(1,change_plot1:k_frame),Y_Orbita_tot(1,change_plot1:k_frame),Z_Orbita_tot(1,change_plot1:k_frame),'Color','m','LineWidth',2.0)
        
        % Plot vettore velocità satellite su orbita_2
        [~,V_orb_i] = ParOrb2RV(a_b2,e_b2,i_f,OM_f,om_2, theta_vect_tot(k_frame), mu_Earth);
        V_vector = quiver3(X_Orbita_tot(k_frame),Y_Orbita_tot(k_frame),Z_Orbita_tot(k_frame),V_orb_i(1),V_orb_i(2),V_orb_i(3),1000,'Color','k','LineWidth',2.0);
    ...
    elseif (k_frame > change_plot2) && (k_frame < change_plot3)
        ...
        [~,V_orb_i] = ParOrb2RV(a_f,e_f,i_f,OM_f,om_3, theta_vect_tot(k_frame), mu_Earth);
        V_vector = quiver3(X_Orbita_tot(k_frame),Y_Orbita_tot(k_frame),Z_Orbita_tot(k_frame),V_orb_i(1),V_orb_i(2),V_orb_i(3),1000,'Color','k','LineWidth',2.0);
...
filename = 'Animated_ManCB.gif';       % Fix the local Path: '...\MediaOpt\Animated_orb_i.gif'
for k_frame = 1:length(theta_vect_frame)
