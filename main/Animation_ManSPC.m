...
run('Plot_ManSPC.m')                        % Plot statico manovra

% Vettore Theta Orbita_tot percorsa
theta_vect_tot = [theta_vect_i;theta_vect_man1;theta_vect_man2;theta_vect_man3;theta_vect_mancirc;theta_vect_f];
...
change_plot2 = change_plot1 + length(theta_vect_man2);
change_plot3 = change_plot2 + length(theta_vect_man3);
change_plotc = change_plot3 + length(theta_vect_mancirc);
change_plotf = change_plotc + length(theta_vect_f);

% Vettori plot Orbita_totale
X_Orbita_tot = [X_Orbita_i,X_Orbita_1,X_Orbita_2,X_Orbita_3,X_Orbita_circ,X_Orbita_f];
Y_Orbita_tot = [Y_Orbita_i,Y_Orbita_1,Y_Orbita_2,Y_Orbita_3,Y_Orbita_circ,Y_Orbita_f];
Z_Orbita_tot = [Z_Orbita_i,Z_Orbita_1,Z_Orbita_2,Z_Orbita_3,Z_Orbita_circ,Z_Orbita_f];
...
ylabel('Y [km]')
zlabel('Z [km]')
title('3D animation Manovra SPC')
view( [1 1 1] )
...
plot3(X_Orbita_1_cmpl,Y_Orbita_1_cmpl,Z_Orbita_1_cmpl,'LineWidth',1.0,'LineStyle',':','Color','c')      % Orbita_1 completa
plot3(X_Orbita_1(1),Y_Orbita_1(1),Z_Orbita_1(1),'o','MarkerEdgeColor','c')                              % Punto inizio percorso Orbita_1
plot3(X_Orbita_2_cmpl,Y_Orbita_2_cmpl,Z_Orbita_2_cmpl,'LineWidth',1.0,'LineStyle',':','Color','g')      % Orbita_2 completa
plot3(X_Orbita_2(1),Y_Orbita_2(1),Z_Orbita_2(1),'o','MarkerEdgeColor','g')                              % Punto inizio percorso Orbita_2
plot3(X_Orbita_3_cmpl,Y_Orbita_3_cmpl,Z_Orbita_3_cmpl,'LineWidth',1.0,'LineStyle',':','Color','m')      % Orbita_3 completa
plot3(X_Orbita_3(1),Y_Orbita_3(1),Z_Orbita_3(1),'o','MarkerEdgeColor','m')                              % Punto inizio percorso Orbita_3
...
plot3(X_Orbita_f_cmpl,Y_Orbita_f_cmpl,Z_Orbita_f_cmpl,'LineWidth',1.0,'LineStyle',':','Color','b')      % Orbita_f completa
...
for k_frame = 1:length(theta_vect_frame)    
    ...
    elseif (k_frame > change_ploti) && (k_frame < change_plot1)
        % Plot orbita_1
        plot3(X_Orbita_tot(1,change_ploti:k_frame),Y_Orbita_tot(1,change_ploti:k_frame),Z_Orbita_tot(1,change_ploti:k_frame),'Color','c','LineWidth',2.0)
        
        % Plot vettore velocità satellite su orbita_1
        [~,V_orb_i] = ParOrb2RV(a_t,e_t,i_i,OM_i,om_i,theta_vect_tot(k_frame), mu_Earth);
        V_vector = quiver3(X_Orbita_tot(k_frame),Y_Orbita_tot(k_frame),Z_Orbita_tot(k_frame),V_orb_i(1),V_orb_i(2),V_orb_i(3),1000,'Color','k','LineWidth',2.0);
        
    elseif (k_frame > change_plot1) && (k_frame < change_plot2)
        % Plot orbita_2
        plot3(X_Orbita_tot(1,change_plot1:k_frame),Y_Orbita_tot(1,change_plot1:k_frame),Z_Orbita_tot(1,change_plot1:k_frame),'Color','g','LineWidth',2.0)
        
        % Plot vettore velocità satellite su orbita_2
        [~,V_orb_i] = ParOrb2RV(a_f,e_f,i_i,OM_i,om_i,theta_vect_tot(k_frame), mu_Earth);
        V_vector = quiver3(X_Orbita_tot(k_frame),Y_Orbita_tot(k_frame),Z_Orbita_tot(k_frame),V_orb_i(1),V_orb_i(2),V_orb_i(3),1000,'Color','k','LineWidth',2.0);
        
    elseif (k_frame > change_plot2) && (k_frame < change_plot3)
        % Plot orbita_3
        plot3(X_Orbita_tot(1,change_plot2:k_frame),Y_Orbita_tot(1,change_plot2:k_frame),Z_Orbita_tot(1,change_plot2:k_frame),'Color','m','LineWidth',2.0)
        
        % Plot vettore velocità satellite su orbita_3
        [~,V_orb_i] = ParOrb2RV(a_f,e_f,i_f,OM_f,om_2,theta_vect_tot(k_frame), mu_Earth);
        V_vector = quiver3(X_Orbita_tot(k_frame),Y_Orbita_tot(k_frame),Z_Orbita_tot(k_frame),V_orb_i(1),V_orb_i(2),V_orb_i(3),1000,'Color','k','LineWidth',2.0);
       
    elseif (k_frame > change_plot3) && (k_frame < change_plotc)
        % Plot orbita_c
        plot3(X_Orbita_tot(1,change_plot3:k_frame),Y_Orbita_tot(1,change_plot3:k_frame),Z_Orbita_tot(1,change_plot3:k_frame),'Color','y','LineWidth',2.0)
        
        % Plot vettore velocità satellite su orbita_c
        [~,V_orb_i] = ParOrb2RV(r_anChange,0,i_f,OM_f,om_2, theta_vect_tot(k_frame), mu_Earth);
        V_vector = quiver3(X_Orbita_tot(k_frame),Y_Orbita_tot(k_frame),Z_Orbita_tot(k_frame),V_orb_i(1),V_orb_i(2),V_orb_i(3),1000,'Color','k','LineWidth',2.0);          
        
    elseif (k_frame > change_plotc) && (k_frame < change_plotf)
        % Plot orbita_f
        plot3(X_Orbita_tot(1,change_plotc:k_frame),Y_Orbita_tot(1,change_plotc:k_frame),Z_Orbita_tot(1,change_plotc:k_frame),'Color','b','LineWidth',2.0)
...
filename = 'Animated_ManSPC.gif';       % Fix the local Path: '...\MediaOpt\Animated_orb_i.gif'
for k_frame = 1:length(theta_vect_frame)
